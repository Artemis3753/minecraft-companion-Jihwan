...
The HTTP contract between `client/` and `server/`. It was fixed before either
side was built, so each was written against a settled contract rather than
against the other. See [`README.md`](../README.md) to run the project and
[`PRD.md`](PRD.md) for scope and the reasoning behind the technical decisions.
...
Whether this back end is running. Setup uses it to confirm the process came up.
...
It answers for the back end alone and never opens an RCON connection, so a `200`
here says nothing about whether the Minecraft server is reachable. That question
belongs to `GET /api/players` and its `503`.
...
unfiltered and ungrouped (see [`PRD.md`](PRD.md)), so the client does nothing per
...
[`README.md`](../README.md) lists every key and where its value comes from.
These three are the ones whose shape was a decision rather than a setting.
...
**The `VITE_` prefix on `VITE_API_BASE_URL` is required:** Vite exposes a
variable to browser code only if it carries that prefix. Anything placed in
`client/.env` ends up in the built bundle and is readable by anyone who loads the
page, so that file holds values that vary by environment, never secrets. That is
the opposite of `server/.env`, which exists precisely to keep secrets off the
wire.
